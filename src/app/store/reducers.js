...
export const itemsReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case SELL_TO_CUSTOMER: {
      const itemIndex = findIndexByItemValue(state[action.item.name], action.item.value);
      const quantity = state[action.item.name][itemIndex].quantity;
      const resultQuantity = +quantity - +action.item.quantity;
      if (+resultQuantity === 0) {
        const afterItemRemoved = update(state, { [action.item.name]: { $splice: [[itemIndex, 1]] } });
        return update(afterItemRemoved, afterItemRemoved[action.item.name].length === 0 ? { $unset: [action.item.name] } : {});
      }
      return update(state, {
        [action.item.name]: {
          [itemIndex]: {
            quantity: { $set: resultQuantity }
          }
        }
      });
    }
